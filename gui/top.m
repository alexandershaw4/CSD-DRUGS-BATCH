...
%
% Work in progress gui for [batch] CSD DCMs
%
% See also MODELS.m Design.m
%
% AS
...
% GET INPUT TEXTFILE
[FILENAME, PATHNAME] = uigetfile('*.txt','Select textfile with list of datasets');
handles.Files = ReadverifyDatasets([PATHNAME '/' FILENAME]);
...
% GET MODEL ARCHS FILE (.m)
[mFILENAME, mPATHNAME] = uigetfile('*.m','Select models file (.m)');
handles.mPath  = mPATHNAME;
handles.MODELS = mFILENAME;
...
% RETURN TRIAL CODE
handles.TrialCode = str2double(get(hObject,'String'));
...
% RETURN ANALYSIS TYPE (CSD)
handles.Anal = (get(hObject,'String'));
...
% RETURN SPATIAL MPDEL: LFP
handles.Spatial = (get(hObject,'String'));
...
% REUTN NEURAL MPODEL
handles.Neural = get(hObject,'String');
...
% invert models on cluster button
%--------------------------------
...
% RETURN DESIGN SCRIPT
% GET MODEL ARCHS FILE (.m)
[mFILENAME, mPATHNAME] = uigetfile('*.m','Select design file (.m)');
handles.dPath  = mPATHNAME;
handles.DESIGN = mFILENAME;
