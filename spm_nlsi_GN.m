...
for k = 1:M.Nmax
    ...
    if exist('Fsi', 'var')
        ...
        xLab = 'time (seconds)';
        try
            if length(M.Hz) == ns || length(M.Hz) == (length(spm_vec(Y.y))/nr)
                x    = Y.Hz;
                x    = linspace(x(1),x(end),ns); % why does FS(y) increase its length?
                xLab = 'Frequency (Hz)';
            end
        end
        ...
        if isreal(spm_vec(y))
            ...
            try M.ShowNodes ... Alex option: subplot node predictions
                ...
                plot(x,real(f + e),'b','LineWidth',2), hold off
                xlabel(xLab)
                ylabel('real')
                title(tstr,'FontSize',16)
                grid on
                ...
                subplot(2,2,2)
                plot(x,imag(f),'r'), hold on
                plot(x,imag(f + e),'b:'), hold off
                xlabel(xLab)
                ylabel('imaginary')
                title(tstr,'FontSize',16)
                grid on
        ...
        % alex - dynamic parameter naming
        try p_l = DCMVECNAMES(M,1);
            set(gca, 'XTick',1:length(p_l), 'XTickLabel',p_l);
        end
